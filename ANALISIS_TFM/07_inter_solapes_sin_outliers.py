...
OUTPUT_CSV = os.path.join(
    BASE_DIR,
    "resultados",
    "inter_solapes_sin_outliers.csv"
)
...
for url_a in urls:
        ...
        for idx_a, p_a in enumerate(puntos_a):
            for idx_b, p_b in enumerate(puntos_b):
                ...
                if d == 0:
                    solape_exacto = "Sí"
                    punto_solape_exacto = (
                        f"{url_a} access_id={grupo_a.iloc[idx_a]['access_id']} "
                        f"<-> {url_b} access_id={grupo_b.iloc[idx_b]['access_id']} "
                        f"punto=({p_a[0]}, {p_a[1]}, {p_a[2]})"
                    )
    ...
    resultados.append({
        ...
        "punto_solape_exacto": punto_solape_exacto
    })
...
print("\nTABLA INTER-CLASE SIN OUTLIERS\n")
print(res.head(30))

print(f"\n[OK] Guardado en: {OUTPUT_CSV}")

print("\nResumen riesgo_solape:")
print(res["riesgo_solape"].value_counts())

print("\nResumen solape_exacto:")
